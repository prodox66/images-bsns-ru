...
- `GET api.php?type=resources&kind=resource&id=...` — WebP 80% до 2048 px по длинной стороне, если производная подготовлена; иначе оригинал.
- `GET api.php?type=resources&kind=thumbnail&id=...` — сохранённый WebP-эскиз.
- `GET api.php?type=resources&kind=original&id=...` — неизменённый исходник для кропера, скачивания и других полноразмерных операций.
...
Пакетные операции ограничены одновременно `operation_batch_size` и `operation_time_budget_seconds`. Одна кнопка обрабатывает только короткую порцию, показывает остаток и блокирует повторный запуск до ответа. Оригинал при оптимизации не удаляется: WebP хранится отдельной производной версией. Профиль качества и размера входит в каталог производной, поэтому новая политика не принимает старый полноразмерный WebP как готовый.
